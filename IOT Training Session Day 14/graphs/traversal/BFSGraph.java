...
class GraphBFS{
    public List<Integer> bfsGraph(int V, List<List<Integer>> adjList){
        ...
        boolean[] visited=new boolean[V+1]; // works for 0mor 1
        Queue<Integer> q=new LinkedList<>();
        // start thr BFS from node 1
        visited[1]=true;
        q.add(1);
        while(!q.isEmpty()){
            int node=q.poll();
            ...
            for(int neighbours:adjList.get(node)) {
                if (!visited[neighbours]) {
                    visited[neighbours] = true; // mark as visited
                    q.add(neighbours); // push to queue
                }
            }
        }
        return bfs;
...
public class BFSGraph {
    public static void main(String[] args) {
        ...
        // if we use this structure List<List<>>, we need to initialize to empty lists first
